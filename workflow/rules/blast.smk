rule fa_split:
    ...
    output:
        temp(expand("../results/trinity/{exp}_filtered_contigs.{num}.fasta", exp=EXP, num=NUM)),
        temp(expand("../results/trinity/{exp}_filtered_contigs.fasta.flat", exp=EXP)),
        temp(expand("../results/trinity/{exp}_filtered_contigs.fasta.gdx", exp=EXP))
    conda:
        "../envs/blast.yml"
    shell:
...
rule blast:
    input:
        expand("../results/trinity/{exp}_filtered_contigs.{{num}}.fasta", exp=EXP)
    output:
